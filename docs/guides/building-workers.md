...
|---|---|---|
| `_worker_id` | `str` | Must match the `id` field in `atlas.yaml`. |
| `_worker_name` | `str` | Must match the `name` field in `atlas.yaml`. |
| `_worker_version` | `str` | Must match the `version` field in `atlas.yaml`. |
| `_worker_roles` | `list[str]` | Must match the `roles` field in `atlas.yaml`. |
...
- **Build an Adapter**: See [Building Adapters](#) for format translation workers.
- **Compose a Manager**: See [Building Managers](building-managers.md) to orchestrate multiple workers into an application.
