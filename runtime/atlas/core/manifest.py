...
class ManifestLoader:
    ...
    def load_file(self, path: str) -> Result[WorkerManifest, Exception]:
        ...
        try:
            ...
            imports = []
            for imp in data.get("imports", []) if isinstance(data.get("imports"), list) else data.get("imports", {}).get("capabilities", []):
                if isinstance(imp, dict):
                    imports.append(ImportDefinition(
                        capability_name=imp.get("name", imp.get("capability", "")),
            ...
            exports = []
            for exp in data.get("exports", []) if isinstance(data.get("exports"), list) else data.get("exports", {}).get("capabilities", []):
                if isinstance(exp, dict):
                    exports.append(ExportDefinition(
                        capability_name=exp.get("name", exp.get("capability", "")),
            ...
            manifest = WorkerManifest(
                ...
                version=str(worker_data.get("version", "")),
                language=worker_data.get("language", ""),
                roles=worker_data.get("roles", []),
                execution=execution,
    ...
    def load_directory(self, path: str) -> Result[List[WorkerManifest], ParseError]:
        """Loads all atlas.yaml files of kind 'worker' in a directory."""
        ...
        for root, _, files in os.walk(path):
            if "atlas.yaml" in files:
                ...
                # Ignore non-worker atlas.yaml files silently during directory scan
                if res.is_ok():
